...
from model.autoencoder_cl import build_model
...
def main(main_args):
    ...
    if dataset_name == 'streamspot' or dataset_name == 'wget':
        ...
        torch.save(model.state_dict(), "./checkpoints/checkpoint-{}_fr{}_sr{}_gcl{}_d{}_l{}.pt".format(
            dataset_name,main_args.ncl,main_args.sr,main_args.gcl, main_args.num_hidden,main_args.num_layers))
    else:
        ck_path = "./checkpoints_cl/checkpoint-{}_ncl{}_sr{}_gcl{}_d{}_l{}_v{}_s{}_n{}_epoch{}_alp{}_bta{}.pt".format(
            dataset_name, main_args.ncl, main_args.sr, main_args.gcl, main_args.num_hidden, main_args.num_layers, main_args.views, main_args.strategy,
            main_args.nid_batch_size, main_args.max_epoch, main_args.alpha, main_args.beta)

        if os.path.exists(ck_path):
            print("file exists: ", ck_path)
            exit(-1)
        ...
        for epoch in epoch_iter:
            ...
            for i in range(n_train):
                    ...
                    for idx in range(seeds_emb.shape[0]):
                        for idy in range(idx + 1, seeds_emb.shape[0]):
                            ...
                            if args.ncl == 1:
                                ...
                                ncl_loss = (contrast_loss_fn(pred1, labels) + contrast_loss_fn(pred2, labels)) / 2
                            else:
                                ncl_loss = torch.tensor([0.]).to(device)
                            ...
                            if args.gcl == 1:
                                ...
                                gcl_loss = (contrast_loss_fn(pred3, labels) + contrast_loss_fn(pred4, labels)) / 2
                            else:
                                gcl_loss = torch.tensor([0.]).to(device)
                            train_contrastive_loss += (1- main_args.beta) * ncl_loss + main_args.beta * gcl_loss
                # print("train_contrastive_loss: ", train_contrastive_loss)
                loss = model(g)
                loss /= n_train
                # total_loss = args.beta * loss + train_contrastive_loss
                if (args.gcl == 1 or args.ncl==1) and args.sr == 1:
                    total_loss = (1 - args.alpha) * loss + args.alpha * train_contrastive_loss
                elif (args.gcl == 1 or args.ncl==1):
                ...
                optimizer.step()
                del g
            print("mem usage : ", psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024, "MB")
